...
2017-10-13 - Dropping ViewThruConversions
...
## depr schedule: https://developers.google.com/adwords/api/docs/sunset-dates
...
#S_API_VERSION = "v201702"
#S_API_VERSION = "v201710"
#S_API_VERSION = "v201802" 
S_API_VERSION = "v201806" 

print "\n\n<<< Using Google API Version %s >>>\n\n" % S_API_VERSION
...
def _convertSudsToOldStyle(o):
    ## pre-201806/googleads 11 required this instead of the zeep
    #dO = dict(o)

    ## post 201806/googleads 11+ requires convert from zeep object to a true dict
    dO = zeep.helpers.serialize_object(o, target_cls=dict)
...
dReportSpecs = {
    'account':{
        ...
        'report_spec':{                     
          ...
          'downloadFormat': 'CSV',
          'selector': {
              #'fields': ['AccountDescriptiveName', 'Date', 'AccountCurrencyCode', 'ExternalCustomerId','AccountTimeZone',
              ...
              'fields': ['AccountDescriptiveName', 'Date', 'AccountCurrencyCode', 'ExternalCustomerId','AccountTimeZone',
                            'Impressions','Clicks','Ctr','AverageCpc','Cost','AveragePosition','AdNetworkType1', 'AdNetworkType2', 'Device', 'ClickType','Conversions','CostPerConversion', 'ConversionRate', 'ConversionValue', 'ValuePerConversion'],
    ...
    'campaign':{
        ...
        'report_spec': {                     
          ...
          'downloadFormat': 'CSV',
          'selector': {
              'fields': ['ExternalCustomerId', 'Date', 'AccountDescriptiveName', 'AccountCurrencyCode', 'AccountTimeZone', 
                    ...
                    # 11/2016 - Google changed schema from ConvertedClicks to Conversions
                    'Conversions','CostPerConversion', 'ConversionRate', 'ConversionValue', 'ValuePerConversion',
                    'AdNetworkType1', 'AdNetworkType2', 'ClickType'],  
            ...
            if bFileExists:
                ...
                #for i, row in enumerate(csvreader):
                    ...
                    if len(row) != 0 and i != 0:
                        ...
                        print sPseudoId in dExisting[sDate].keys()
                        #print row
                        o = None
    ...
    for sDate in lsSDates:
        ...
        with open(file_path, 'rb') as csvfile:
            ...
            for i, row in enumerate(csvreader):
                ...
                if len(row) != 0 and i != 0:
                    sPseudoId = row[dHeaderLookup['call_id']]
                    latestIds.add(sPseudoId)
                    #print sPseudoId
                    #print row
                    o = None
                    ...
                    if sPseudoId not in dExisting[sDate]:
                        ...
                        o, bCreated = models.VoiceStarCallLog.objects.get_or_create(call_id__exact=row[dHeaderLookup['call_id']])
                    ...
                    if o:
                        ...
                        o.caller_number = row[dHeaderLookup['caller_number']]
                        o.caller_name = row[dHeaderLookup['caller_name']]
                        o.call_id = row[dHeaderLookup['call_id']]
                        o.inbound_no = row[dHeaderLookup['inbound_no']]
                        o.inbound_ext = row[dHeaderLookup['inbound_ext']][:25]  # quick fix for asshats who post not legit ext
                        o.keyword = row[dHeaderLookup['keyword']]
                        o.forward_no = row[dHeaderLookup['forward_no']]
                        o.custom_id = row[dHeaderLookup['custom_id']]
                        o.account_id = row[dHeaderLookup['account_id']]
                        o.group_id = row[dHeaderLookup['group_id']]
                        o.campaign_id = row[dHeaderLookup['campaign_id']]
                        o.a_name = row[dHeaderLookup['a_name']]
                        o.g_name = row[dHeaderLookup['g_name']]
                        o.c_name = row[dHeaderLookup['c_name']]
                        #!!!

                        dtLocalTime = dateutil.parser.parse(row[dHeaderLookup['call_s']])
                        dtCall = toUTC(dateutil.parser.parse(row[dHeaderLookup['call_s']]), etz)
                        o.call_s = dtCall
                        sDTRange.add(dtCall)

                        lsDurationParts = [int(x) for x in row[dHeaderLookup['duration']].split(":")]
                        iDuration = 60*60*lsDurationParts[0] + 60*lsDurationParts[1] + lsDurationParts[2]
                        o.duration = iDuration

                        lsOffsetParts = [int(x) for x in row[dHeaderLookup['answer_offset']].split(":")]
                        iOffset = 60*60*lsOffsetParts[0] + 60*lsOffsetParts[1] + lsOffsetParts[2]
                        o.answer_offset = iOffset

                        o.call_status = row[dHeaderLookup['call_status']]
                        o.disposition = row[dHeaderLookup['disposition']]
                        o.rating = row[dHeaderLookup['rating']]
        ...
        if sType== "account":
                    ...
                    if row[0].strip() != "Total" and i not in [0,1]:
                        ...
                        if o:
                            ...
                            try:
                                ...
                                o.ViewThroughConversions = 0 # int(row[19])
                                o.ValuePerConversion = float(row[19].replace(",",""))  # was known as ValuePerConvertedClick
            ...
            with open(file_path, 'rb') as csvfile:
                ...
                for i, row in enumerate(csvreader):
                    ...
                    if row[0].strip() != "Total" and i not in [0,1]:
                        #print parser.parse(row[1]).date(), existingDates, sCustId
                        sPseudoId = unicode(row[5], 'utf-8') + u"_" + u"_".join([unicode(x, 'utf-8') for x in row[19:22]])
                        o = None
                        if parser.parse(row[1]).date() not in dExisting.keys() or sPseudoId not in dExisting[parser.parse(row[1]).date()]:
                            #print parser.parse(row[1]).date(), existingDates, sCustId
                            #Account, Day, Currency, Account ID, Time zone, Impressions, Clicks, CTR, Avg. CPC, Cost, Avg. position
                            ...
                            o, bCreated = models.GoogleCampaignPerformanceReport.objects.get_or_create(ExternalCustomerId__exact=int(sCustId),Date__exact=dateutil.parser.parse(sDate).date(), 
                                CampaignId__exact=int(row[5]),AdNetworkType1__exact=row[19], AdNetworkType2__exact=row[20],ClickType__exact=row[21])
                            print "ere", bCreated, int(row[5]), row[19], row[20], row[21]
                            logging.warning("got it here")
                        ...
                        if o:
                            ...
                            o.ViewThroughConversions = 0 # int(row[18])
                            o.ValuePerConversion = float(row[18].replace(",",""))  # now known as ValuePerConvertedClick
                            o.AdNetworkType1 = row[19]
                            o.AdNetworkType2 = row[20]
                            #o.Device = row[14]
                            o.ClickType = row[21]
                            try:
                                o.save()
            ...
            with open(file_path, 'rb') as csvfile:
                ...
                for i, row in enumerate(csvreader):
                    ...
                    if row[0].strip() != "Total" and i not in [0,1]:
                        ...
                        sPseudoId = unicode(row[8], 'utf-8') + "_" + unicode(row[11], 'utf-8') +u"_" +  u"_".join([unicode(x, 'utf-8') for x in row[25:29]])
                        o = None
                        if parser.parse(row[1]).date() not in dExisting.keys() or sPseudoId not in dExisting[parser.parse(row[1]).date()]:
                            #print parser.parse(row[1]).date(), existingDates, sCustId
                            #Account, Day, Currency, Account ID, Time zone, Impressions, Clicks, CTR, Avg. CPC, Cost, Avg. position
                            ...
                            o, bCreated = models.GoogleKeywordPerformanceReport.objects.get_or_create(ExternalCustomerId__exact=int(sCustId),
                                            ...
                                            AdNetworkType1__exact=row[25], 
                                            AdNetworkType2__exact=row[26],
                                            Device__exact=row[27],
                                            ClickType__exact=row[28])
                            print "ere", bCreated, int(row[8]), row[11], row[25], row[26],row[27],row[28]
                            logging.warning("got it here")
                        ...
                        if o:
                            ...
                            o.ViewThroughConversions = 0 # int(row[24])
                            o.ValuePerConversion = float(row[24].replace(",",""))  # now known as ValuePerConvertedClick
                            o.AdNetworkType1 = row[25]
                            o.AdNetworkType2 = row[26]
                            o.Device = row[27]
                            o.ClickType = row[28]
                            try:
                                o.save()
            ...
            with open(file_path, 'rb') as csvfile:
                ...
                for i, row in enumerate(csvreader):
                    ...
                    if row[0].strip() != "Total" and i not in [0,1]:
                        ...
                        sPseudoId = unicode(row[8], 'utf-8') +u"_" + u"_".join([unicode(x, 'utf-8') for x in row[11:16]]) + u"_" + u"_".join([unicode(x, 'utf-8') for x in row[27:30]])
                        if parser.parse(row[1]).date() not in dExisting.keys() or sPseudoId not in dExisting[parser.parse(row[1]).date()]:
                            ...
                            o.ViewThroughConversions = 0 # int(row[26])
                            o.ValuePerConversion = float(row[26].replace(",",""))  # now known as ValuePerConvertedClick
                            o.AdNetworkType1 = row[27]
                            o.AdNetworkType2 = row[28]
                            o.Device = row[29]
                            #o.ClickType = row[15]
                            try:
                                o.save()
